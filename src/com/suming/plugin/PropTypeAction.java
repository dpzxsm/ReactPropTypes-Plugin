...
import com.suming.plugin.constants.SpecialPropTypes;
            ...
            if ("shape".equals(beans.get(i).type)) {
                ...
                if (shapePropList != null) {
                    ...
                    for (int j = 0; j < shapePropSize; j++) {
                        ...
                        final String propType = shapePropList.get(j).type;
                        boolean isSpecial = Arrays.stream(SpecialPropTypes.values()).anyMatch(o -> o.name().equals(propType));
                        if (isSpecial) {
                            sb.append("(");
                            if(shapePropList.get(j).getJsonData() != null){
                                sb.append(shapePropList.get(j).getJsonData());
                            }
                            sb.append(")");
                        }
                ...
                final String propType = beans.get(i).type;
                boolean isSpecial = Arrays.stream(SpecialPropTypes.values()).anyMatch(o -> o.name().equals(propType));
                sb.append(beans.get(i).type);
                if (isSpecial) {
                    sb.append("(");
                    if(beans.get(i).getJsonData() != null){
                        sb.append(beans.get(i).getJsonData());
                    }
                    sb.append(")");
                }
            }
